          ...
          <div class="w-full">
              ...
              <div>
                ...
                <div class="flex items-center justify-center gap-2 p-3 bg-gray-50 rounded-lg">
                  ...
                  <button
                    class="text-gray-400 hover:text-gray-600 transition-colors"
                    title="Copy address"
                    @click="copyToClipboard(claim?.poolAddress)"
                  >
                    <svg
                      class="h-4 w-4"
                      fill="none"
            ...
            <div class="space-y-4">
              ...
              <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                ...
                <div
                  v-if="!claim?.approved && !claim?.spam && !claim?.executed"
                  class="space-y-2"
                >
                  <div class="flex justify-between items-center">
                    ...
                    <span
                      class="text-sm font-medium"
                      :class="canExecuteApproval ? 'text-green-600' : 'text-gray-900'"
                    >
                  ...
                  <div class="flex justify-between items-center">
                    ...
                    <span
                      class="text-sm font-medium"
                      :class="canExecuteSpam ? 'text-red-600' : 'text-gray-900'"
                    >
                ...
                <!-- Timing Information -->
                <div
                  v-if="claim?.approved && !claim?.executed && !isReadyForExecution"
                  class="flex justify-between items-center"
                >
                  <span class="text-sm text-gray-600">Execution</span>
                  <span
                    class="text-sm font-medium"
                    :class="isReadyForExecution ? 'text-blue-600' : 'text-yellow-600'"
                  >
            ...
            <!-- Actions -->
            <div
              v-if="!claim?.executed && !claim?.spam"
              class="flex justify-end gap-4 pt-6"
            >
              ...
              <template v-else-if="!claim?.approved && isApprovalPeriodActive && props.isController">
                <button
                  class="px-6 py-2.5 bg-red-500 border border-red-500 text-white text-lg rounded-xl hover:bg-white hover:text-red-500 hover:border-red-500 transition-colors duration-300"
                  @click="$emit('mark-spam', claim?.id)"
                >
                  Vote Spam
                </button>
                <button
                  class="btn-primary px-6 py-2.5 text-lg rounded-xl"
                  @click="$emit('approve', claim?.id)"
                >
                  Vote Approve
                </button>
              </template>
              ...
              <button
                v-else-if="claim?.approved && !claim?.executed"
              ...
              <div
                v-else-if="!claim?.approved && !isApprovalPeriodActive"
...
const getActualPayoutAmount = computed(() => {
  ...
  const reductionFactor = (BASIS_POINTS - BigInt(props.claimReduction));
  const reducedAmount = (BigInt(props.claim.amount) * reductionFactor) / BASIS_POINTS;
...
const getReductionPercentage = computed(() => {
  ...
  return (Number(props.claimReduction) / 100).toFixed(2);
});
